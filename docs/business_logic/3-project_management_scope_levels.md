...
> [!IMPORTANT]
> These levels describe the **management scope**, and do not automatically determine job titles or system permissions. A Sponsor may govern a specific project; a Scrum Master does not by default have personnel evaluation rights. Refer to `project_governance_raci.md` to define RACI/permissions and `project_management_operating_model.md` to define the lifecycle.
